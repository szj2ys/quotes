...
function App() {
    ...
    // 合并空格计算逻辑为一个函数
    const calculateSpaces = useCallback((authorLength, ratio) => {
    ...
    // 检查对象是否为空
    const isQuoteEmpty = useCallback((quote) => Object.keys(quote).length === 0, []);

    // 初始化名言展示逻辑
    const initializeQuotes = useCallback(() => {
    ...
    }, [isMobile, handleDoubleClick, handleKeyDown]);
